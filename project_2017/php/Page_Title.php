<html>
    <head>
        <meta charset="utf-8">
        <title>MuSik</title>
        <link href="../css/Design.css" type=text/css rel=stylesheet>
        <link href="../css/Design_Bar.css" type=text/css rel=stylesheet>
        <link href="../css/Design_Main.css" type=text/css rel=stylesheet>
    </head>
    <body>
        <header>
        <div class = "bar"><!-- menu bar-->
            <div class = "title">
                <img src = "../source/logo.png" width = 70px height = 35px style = "float: left; background-color: black;">
                <p>MuSik</p>
            </div>
            <div class = "menu">
                <span><a class = "menu_link" href = "Page_Title.php">HomePage</a></span>
                <span><a class = "menu_link" href = "Page_Introduce.php">Introduce</a></span>
                <span><a class = "menu_link" href = "Page_Upload.php">Upload</a></span>
            
            <div class = "loginforad">
                <div><a class = "login_link" href = "Page_Login.php">Login for admin</a></div>
            </div>
            </div>
        </div>

        <div class = "submenu_tag">
                <a class = "submenu_link" href = "#Rock">Rock</a>
                <a class = "submenu_link" href = "#Classical" style = "left: 275px;">Classical</a>
                <a class = "submenu_link" href = "#Jazz" style = "left: 550px;">Jazz</a>
                <a class = "submenu_link" href = "#Pop" style = "left: 825px;">Pop</a>
        </div>
        <div class = "submenu_tag" style = "top: 175px;">
                <a class = "submenu_link" href = "#Ballad">Ballad</a>
                <a class = "submenu_link" href = "#Hiphop" style = "left: 275px;">Hiphop</a>
                <a class = "submenu_link" href = "#Elec" style = "left: 550px;">Elec</a>
                <a class = "submenu_link" href = "#RnB" style = "left: 825px;">RnB</a>
        </div>
        <div class = "submenu_tag" style = "top: 222px;">
                <a class = "submenu_link" href = "#Game">Game</a>
                <a class = "submenu_link" href = "#Drama" style = "left: 275px;">Drama</a>
                <a class = "submenu_link" href = "#Movie" style = "left: 550px;">Movie</a>
                <a class = "submenu_link"href = "#ETC" style = "left: 825px;">ETC</a>
        </div>
        </header>

        <section style = "top: 300px;">
                <div class = "array_sound">
                        <p>&nbsp;Spectre <audio src = "../music/Alan Walker - Spectre NCS Release.mp3" controls>NULL</audio></p>
                        <p class = "tag">&nbsp;&nbsp;&nbsp;Elec, ETC</p>
                        <hr width = 1400px />
                        음악 배치
                        <hr/>
                        음악 배치
                        <hr/>
                        음악 배치
                        <hr/>
                        음악 배치
                        <hr/>
                        음악 배치
                        <hr/>
                        음악 배치
                        <hr/>
                        음악 배치
                        <hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        음악 배치<hr/>
                        </div>
        </section>
        <!--
        <aside class = "player_sound">
            음악재생 상황 설명
        </aside>

            <footer class = "last">
                종료
            </footer>-->
            </body>
</html>